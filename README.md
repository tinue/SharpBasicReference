...
- Program capacity: **1850 steps** maximum
...
The commands in this section require the **Sharp CE-150 Printer/Plotter/Cassette Interface** to be physically connected. They will not function on a standalone PC-1500.

For full documentation of the CE-150 printing and plotting commands (`LPRINT`, `LLIST`, `LINE`, `GRAPH`, `COLOR`, `CSIZE`, etc.) see [CE-150-Reference.md](CE-150-Reference.md).

### CSAVE

- **Syntax**: `CSAVE ["filename"]` or `CSAVE-1 ["filename"]`
- **Abbreviation**: CS. CSA. CSAV.

Saves the current program to cassette tape. Filename may be up to 16 characters. `CSAVE-1` uses a second tape recorder. Always verify the save with `CLOAD?`.
...
- **Abbreviation**: CLO. CLOA.

Loads a program from cassette tape, replacing the current program. Searches for the matching filename; `CLOAD` with no filename loads the first program found.
...
Verifies the tape contents against the program in memory without loading. Displays the filename if they match; **ERROR 43** if they differ; **ERROR 44** on checksum error.
...
- **Abbreviation**: MER. MERG.

Loads a program from tape without clearing the current program. Lines from tape are added to memory. Useful for combining program sections.

---

### CHAIN

- **Syntax**: `CHAIN "filename" [, line]` or `CHAIN-1 "filename" [, line]`
- **Abbreviation**: CHA. CHAI.
- **Statement only** — cannot be used as a direct command.

Loads the named program from tape and begins executing it immediately. If a line number is specified, execution starts there. Variables are preserved across the `CHAIN`.

---

### PRINT#

- **Syntax**: `PRINT # [; "filename"] ; var [, var ...]`
- **Abbreviation**: P.# PR.# PRI.# PRIN.#

Saves variable values (not a program) to cassette tape.

---

### INPUT#

- **Syntax**: `INPUT # [; "filename"] ; var [, var ...]`
- **Abbreviation**: I.# IN.# INP.# INPU.#
...
- **Abbreviation**: RM.O. RMTO. (ON); RM.OF. RMTOF. (OFF)

Controls the remote function for a second tape recorder connected to the REM 1 terminal.
...
- **Syntax**: Same forms as `PRINT`
- **Abbreviation**: LP. LPR. LPRI. LPRIN.
- **Requires**: CE-150 in TEXT mode

Prints to the CE-150 printer. Takes exactly the same syntax as `PRINT`, including `;` and `,` separators, `TAB`, and `USING` formatting.
...
- **Syntax**: `LLIST`
- **Abbreviation**: LL. LLI. LLIS.
- **Requires**: CE-150 in TEXT mode

Prints the current BASIC program listing to the CE-150 printer. Equivalent to `LIST` but output goes to the printer instead of the display.
...
- **Syntax**: `TEST`
- **Abbreviation**: TE. TES.
- **Requires**: CE-150 in GRAPH mode

Draws a 5 mm × 5 mm filled square in each of the four pen colors. Used to verify that all pens are functioning and correctly positioned before a plotting session.

---

### CSAVE M

- **Syntax**: `CSAVE M [address1, address2 [, address3]]` or `CSAVE M-1 [...]`
- **Abbreviation**: CS. CSA. CSAV. (for CSAVE)

Saves a block of machine language (raw memory bytes) to cassette tape. `address1` is the start address, `address2` is the end address. If `address3` is given, execution will automatically jump there when the file is loaded.

Adding `-1` selects the REMOTE-1 tape recorder.
...
- **Abbreviation**: CLO. CLOA. (for CLOAD)

Loads a machine language block from tape into the same memory addresses used when it was saved. If `address` is specified, loads starting from that address instead. If the saved file included an auto-run address (expression-3 in CSAVE M), execution jumps there automatically after loading — unless you specify a load address override.
...
The **CE-158** is an optional RS-232C serial / Centronics parallel interface for the PC-1500. When connected, it adds the following BASIC commands. None of these work without the CE-158 attached.

### SETCOM

- **Syntax**: `SETCOM baud, parity, bits, stop`
- **Abbreviation**: none

Configures the RS-232C serial port parameters before communication.
...
Selects the output device for subsequent `PRINT`/`LPRINT` output (e.g. serial port, parallel port, or local display/printer).
...
Puts the PC-1500 into interactive terminal mode, passing keystrokes through the RS-232C port and displaying received characters on the PC-1500 display. Press BREAK to exit.
...
Sends a string over the RS-232C serial port.
...
Opens the RS-232C communications channel.
...
Returns the output status of the communications port. Non-zero if the port is ready to send.
...
Returns the input status of the communications port. Non-zero if a character is waiting to be read.
...
Reads one character received over the RS-232C port without waiting. Returns a space if no character is available (same non-blocking behaviour as `INKEY$`).
...
Communications status function. Returns status information about the CE-158 interface.
...
Returns the current device identifier string for the active output device.
...
Data Terminal Equipment control for the RS-232C interface.

---

*See also: [CE-150-Reference.md](CE-150-Reference.md) for printer/plotter/cassette commands.*
